class Game2048 {
  ...
  getBestMove() {
    ...
    document.getElementById("positions-evaluated").textContent =
  ...
  minimax(grid, depth, isMaximizing, alpha, beta) {
    ...
    if (isMaximizing) {
      ...
      for (const move of moves) {
        ...
        if (this.simulateMove(testGrid, move)) {
          const score = this.minimax(testGrid, depth - 1, false, alpha, beta);
      ...
      return maxScore === -Infinity ? this.evaluateGrid(grid) : maxScore;
    } else {
      ...
      // Simuler l'ajout de tuiles aléatoirement
      for (const cell of emptyCells.slice(0, Math.min(6, emptyCells.length))) {
        for (const value of [2, 4]) {
          const testGrid = grid.map((row) => [...row]);
          testGrid[cell.r][cell.c] = value;

          const score = this.minimax(testGrid, depth - 1, true, alpha, beta);
  ...
  evaluateGrid(grid) {
    ...
    // Score basé sur les valeurs des tuiles
    for (let r = 0; r < 4; r++) {
      for (let c = 0; c < 4; c++) {
        score += grid[r][c];
      }
    ...
    // Bonus pour les cases vides
    score += this.getEmptyCells(grid).length * 100;

    // Bonus pour la monotonie (tuiles organisées)
    score += this.getMonotonicity(grid) * 10;

    // Bonus pour la douceur (différences faibles entre tuiles adjacentes)
    score += this.getSmoothness(grid) * 5;

    // Bonus si la plus grande tuile est dans un coin
    ...
    this.aiInterval = setInterval(() => {
      ...
      if (bestMove) {
        ...
        document.getElementById("status").textContent =
          "IA bloquée - Aucun mouvement possible";
      }
    }, 500);
  }
  ...
  stopAI() {
    ...
    document.getElementById("status").textContent =
      "Mode manuel - Utilisez les flèches ou WASD";
  ...
  reset() {
    ...
    document.getElementById("undo-btn").disabled = true;
    document.getElementById("status").textContent =
      "Utilisez les flèches ou WASD pour jouer";
    document.getElementById("status").className = "status";
  }
}
